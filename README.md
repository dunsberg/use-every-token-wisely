...
Reads the OAuth access token from local credential files (with automatic refresh when needed), then calls `claude.ai/api/oauth/usage` to get real session (5h) and weekly (7d) utilization, plus per-model limits (e.g. Fable). Uses a persistent cookie jar to pass Cloudflare's bot detection.
